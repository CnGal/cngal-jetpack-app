...
fun RelevanceGroupCard(
    ...
    relatedVideos: List<VideoCardModel>,
    outlinks: List<OutlinkModel>,
    name: String,
    onNav: (String) -> Unit
)
{
    ...
    {
        TitleCard(title = "关联视频", onClickLink = {
            val title = URLEncoder.encode("${name}的关联视频", "utf-8")
            val json = JsonHelper.toJson(relatedEntries)
            onNav("${VideoCardGroupDestination.route}/${title}/${json}")
        }, content = {
            LazyRow(
                horizontalArrangement = Arrangement.spacedBy(12.dp),
                contentPadding = PaddingValues(horizontal = 12.dp),
            ) {
                items(items = relatedVideos) { item ->
                    VideoCard(item, false, onNav)
                }
            }
        })
    }
    ...
    {
        TitleCard(title = "外部链接", linkText = null, onClickLink = {}, content = {
            Column(
